-- Expand sessions.agent_kind CHECK for Kiro CLI (AWS).
--
-- Every sessions-table rebuild must retain the complete AgentKind set and the
-- V18/V22 scope-pairing triggers. Without this rebuild the hook router WARNs
-- and drops kiro-cli sessions at begin_session, silently disabling capture.
...
CREATE TABLE sessions_new (
    ...
    agent_kind       TEXT NOT NULL CHECK (agent_kind IN ('claude-code','codex','open-code','cursor','gemini-cli','claude-desktop','openclaw','antigravity-cli','omp','pi','crush','grok','zero','devin','kimi-code','kiro-cli','other')),
    cwd              TEXT,
